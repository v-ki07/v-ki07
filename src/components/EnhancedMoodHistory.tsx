...
interface EnhancedMoodHistoryProps {
  entries: MoodEntry[];
  onDelete: (id: string) => void;
}

export const EnhancedMoodHistory = ({ entries, onDelete }: EnhancedMoodHistoryProps) => {
          ...
          return (
            <Card key={entry.id} className="group border-none shadow-sm bg-white dark:bg-zinc-900">
              <CardContent className="p-6">
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-[10px] text-zinc-500 dark:text-zinc-400 uppercase tracking-widest">
                  ...
                  <Button 
                    ...
                    className="opacity-0 group-hover:opacity-100 h-8 w-8 text-zinc-400 hover:text-red-500 transition-opacity"
                  >
                ...
                {(entry.body_scan || entry.coping_strategy) && (
                  <div className="pt-4 border-t dark:border-zinc-800 mt-4">
                    <h4 className="text-sm font-bold text-zinc-900 dark:text-zinc-100 flex items-center gap-2 mb-2">
                    ...
                    {entry.body_scan && (
                      <div className="mb-2">
                        <p className="text-xs text-zinc-500 dark:text-zinc-400 mb-1">Body Scan:</p>
                        <p className="text-sm text-zinc-700 dark:text-zinc-300">{entry.body_scan}</p>
                      </div>
                    )}
                    
                    {entry.coping_strategy && (
                      <div>
                        <p className="text-xs text-zinc-500 dark:text-zinc-400 mb-1">Coping Strategy:</p>
                        <p className="text-sm text-zinc-700 dark:text-zinc-300">{entry.coping_strategy}</p>
                      </div>
                    )}
                  </div>
                )}
                ...
                {entry.visual_metaphor && (
                  <div className="pt-4 border-t dark:border-zinc-800 mt-4">
                    ...
                    <p className="text-sm text-zinc-700 dark:text-zinc-300 italic">{entry.visual_metaphor}</p>
                  </div>
                )}
              </CardContent>
            </Card>
